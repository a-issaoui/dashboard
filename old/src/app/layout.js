...
import { getDirection, SUPPORTED_LOCALES, DEFAULT_LOCALE } from '@/lib/locale';
import NextTopLoader from 'nextjs-toploader';
import { setSsrInitialState } from '@/store/localeStore';
...
export default async function RootLayout({ children }) {
    ...
    return (
        <html
            ...
            className={cn('h-full scroll-smooth antialiased', fontVariables)}
        >
        <head>
            <script
                dangerouslySetInnerHTML={{
                    __html: `
                        ...
                        window.__SUPPORTED_LOCALES__ = ${JSON.stringify(SUPPORTED_LOCALES)};
                    `,
                }}
            />
            <meta charSet="utf-8" />
