// Notification.jsx - Enhanced with better performance and UX
'use client';
...
const NotificationIcon = memo(({ iconName, className, priority }) => {
    const IconComponent = Icons[iconName] || Icons.Bell;
...
const Notification = memo(() => {
    ...
    const handleBatchActions = useCallback((action, notificationIds) => {
        switch (action) {
            case 'markRead':
    ...
    return (
        <>
            <DropdownMenu open={isMenuOpen} onOpenChange={handleOpenChange}>
                <DropdownMenuTrigger
                    className=" size-8  relative touch-manipulation select-none"
